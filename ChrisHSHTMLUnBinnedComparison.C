...
void ChrisHSHTMLUnBinnedComparison(){
...
//  gSystem->Setenv("PREVFITNAME",PrevFitName);
  ...
  //for(Int_t i=22;i<23;i++){	
    ...
    TFile *ResultFile= TFile::Open(fileDir+"/"+FitName+"Asyms"+JobName+".root");//For Unbinned
...
//    TH1F* result=(TH1F*) ResultFile->Get(JobName+FitVariable+"1");
    TH1F* result=(TH1F*) ResultFile->Get(FitVariable);
    result->Draw();
