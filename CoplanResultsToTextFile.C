...
void CoplanResultsToTextFile() {
		
  TFile *infile = TFile::Open("/w/work3/home/chris/LatestAnalysisRuns/Data/DataJul18/HistoSelector/Pi0Analysis/PPi0ResultBinnedCoplanWithInvMassWeights.root");
  ...
  for (int i=0; i<3; i++) {
	...
	while ((polbinKey=(TKey*)nextPolbinDir())){   //Looping round my polarisation states - +
	  ...
	  TIter	nextCoplanbinDir(polbinDir->GetListOfKeys());
	  TKey	*coplanbinKey;


	while ((coplanbinKey=(TKey*)nextCoplanbinDir())){   //Looping round my coplanarity bins

	  TDirectory *coplanbinDir = polbinDir->GetDirectory(coplanbinKey->GetName());
	  TIter	nextHist(coplanbinDir->GetListOfKeys());
	  TKey	*histKey;

					
	  int histNum = 0;
	  while ((histKey=(TKey*)nextHist())) { // Do another loop for the coplan bins and add this into the names of things    
	    ...
	    if(IsPhi=="Phi" && IsPolPos=="PolStateD-0.50_"){
	      TString PolPosName ="/" + dirs[i]+"/" + (TString)tbinDir->GetName()+ "/"  +  (TString)ebinDir->GetName() + "/PolStateD0.50_/"+ (TString)coplanbinDir->GetName() +"/Phi";
	      TString PolarPosName ="/" + dirs[i]+"/" + (TString)tbinDir->GetName()+ "/"  +  (TString)ebinDir->GetName() + "/PolStateD0.50_/"+ (TString)coplanbinDir->GetName() +"/Polarisation";
	      TString PolarNegName ="/" + dirs[i]+"/" + (TString)tbinDir->GetName()+ "/"  +  (TString)ebinDir->GetName() + "/PolStateD-0.50_/"+ (TString)coplanbinDir->GetName() +"/Polarisation";
	      ...
	      TString CoplanbinName = (TString)coplanbinDir->GetName();		
	      string CoplanbinName2 = (string)CoplanbinName(11,4);
		cout << CoplanbinName<< endl;
		cout << CoplanbinName2 << endl;
	      Double_t Coplanbin = stod(CoplanbinName2);
	      VecAllCoplan.push_back(Coplanbin);		
	      ...
	      VecAllCoplanErr.push_back(10);		

	    } //Closing if 
	  }  //Closing while hist key		
	  }  //Closing while coplan key		
...
textfile.open("MyResultsNewInvCoplanBinned.txt",std::ios_base::app);

for(Int_t ddd=0; ddd<VecAllSigma.size(); ddd++ ){
  textfile <<VecAllSigma[ddd]<<"  "<<VecAllSigmaErr[ddd]<<"  "<< VecAllEg[ddd]<<"  "<<VecAllEgErr[ddd]<<"  "<<VecAllCosth[ddd]<<"  "<<VecAllCosthErr[ddd]<<"  "<<VecFolder[ddd]<<"  "<<VecAllCoplan[ddd]<<"  "<<VecAllCoplanErr[ddd]<<endl;
