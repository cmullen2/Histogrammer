...
#include "CutsTHSSelector_vh.h"
#include <TH2.h>
#include <TStyle.h>

void CutsTHSSelector_vh::Begin(TTree * /*tree*/)
{
...
void CutsTHSSelector_vh::SlaveBegin(TTree * /*tree*/)
{
...
//    THSHisto::LoadWeights("/w/work3/home/chris/LatestAnalysisRuns/Data/DataJul18/Fitting/Pi0Analysis/MergeWeights/ProtonWeightsSum/CoplanPoly2ndOrder/MoreBins/Final/WeightsBothTotal.root","HSsWeights");
...
Bool_t CutsTHSSelector_vh::Process(Long64_t entry)
{
...
void CutsTHSSelector_vh::SlaveTerminate()
{
...
void CutsTHSSelector_vh::Terminate()
{
...
void CutsTHSSelector_vh::HistogramList(TString sLabel){
...
void CutsTHSSelector_vh::FillHistograms(TString sCut){
	...
	if(*AnyErrs==0){
		if(*TopoMine==1){
			if(*BeamEnergy>400 && *BeamEnergy<720){
				if(*GammaAveTagDiffTime<4 && *GammaAveTagDiffTime>-4){
...
//  FindHist("AlsoMissMass")->Fill(*MissMass,fWeight);//1D
  ...
  FindHist("Costh")->Fill(*Costh);//1D
  FindHist("PolState")->Fill(*PolState);//1D
  FindHist("BeamEnergy")->Fill(*BeamEnergy);//1D
  FindHist("PolErrs")->Fill(*PolErrs);
  FindHist("WII")->Fill(*WII);
