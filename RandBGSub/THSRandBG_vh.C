#define THSRandBG_vh_cxx
// The class definition in THSRandBG_vh.h has been generated automatically
...
//
// root> T->Process("THSRandBG_vh.C")
// root> T->Process("THSRandBG_vh.C","some options")
// root> T->Process("THSRandBG_vh.C+")
//


#include "THSRandBG_vh.h"
#include <TH2.h>
#include <TStyle.h>

void THSRandBG_vh::Begin(TTree * /*tree*/)
{
...
void THSRandBG_vh::SlaveBegin(TTree * /*tree*/)
{
 ...
 //  THSHisto::LoadCutsForWeights(); //Loop over weights species and make weighted histograms for each
   ...
   THSHisto::LoadCut("Prompt");
   THSHisto::LoadCut("Random");
   THSHisto::LoadHistograms();

}

Bool_t THSRandBG_vh::Process(Long64_t entry)
{
  ...
  // THSHisto::FillCutsForWeights(); //Fill weighted hists
   ...
   if(*Topo==1){
     if(*Coplanarity<50 && *Coplanarity>-50){
       if(*W<1850 && *W>950){
	 if(*DCorrect==0){
	   if(*GammaAveTagDiffTime<-45 && *GammaAveTagDiffTime>-65){
...
void THSRandBG_vh::SlaveTerminate()
{
...
void THSRandBG_vh::HistogramList(TString sLabel){
...
void THSRandBG_vh::FillHistograms(TString sCut){
