...
void RandBGSubSpecMomBins() { 
  ...
  while ((tbinKey=(TKey*)nextTbinDir())) {
	...
	TKey    *specbinKey;
        TDirectory *outPolBinDir = outEBinDir->mkdir(polbinKey->GetName());

	while ((specbinKey=(TKey*)nextSpec())) {
	  ...
	  while ((histKey=(TKey*)nextHist())) {
	    ...
	    TString histBgName ="/" + dirs[2] + "/" + (TString)tbinDir->GetName()+ "/"  +  (TString)ebinDir->GetName() + "/" + (TString)polbinDir->GetName() + "/" +(TString)specbinDir->GetName()  +"/" + (TString)histKey->GetName() ;
